...
/// Tokens from design.md / plan Visual contract — exact hex values.
class SonderPalette {
  // Dark
...
/// Animation durations that respect reduced-motion preference.
class SonderMotion {
  /// 140ms for standard motion, 0 for reduced motion.
  static Duration standard(BuildContext context) {
    final bool reduce =
        MediaQuery.maybeOf(context)?.accessibleNavigation ?? false;
    // Also respect disableAnimations flag.
    final bool disableAnimations =
        MediaQuery.maybeOf(context)?.disableAnimations ?? false;
    if (reduce || disableAnimations) return Duration.zero;
    return const Duration(milliseconds: 140);
  }

  static Duration tick(BuildContext context) {
    final bool reduce =
        MediaQuery.maybeOf(context)?.accessibleNavigation ?? false;
    final bool disableAnimations =
        MediaQuery.maybeOf(context)?.disableAnimations ?? false;
